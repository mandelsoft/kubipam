...
RELEASE                     := true
NAME                        := kubipam
REPOSITORY                  := github.com/mandelsoft/$(NAME)
...
.PHONY: images-dev
images-dev:
	@docker build -t $(IMAGE_PREFIX)/$(NAME):$(VERSION)-dev-$(COMMIT) -t $(IMAGE_PREFIX)/$(NAME):latest -f Dockerfile -m 6g --build-arg TARGETS=dev --target command .
	@docker push $(IMAGE_PREFIX)/$(NAME):latest

.PHONY: images-release
images-release:
	@docker build -t $(IMAGE_PREFIX)/$(NAME):$(VERSION) -t $(IMAGE_PREFIX)/$(NAME):latest -f Dockerfile -m 6g --build-arg TARGETS=release --target command .
	@docker push $(IMAGE_PREFIX)/$(NAME):latest
	@docker push $(IMAGE_PREFIX)/$(NAME):$(VERSION)

.PHONY: images-release-all
images-release-all:
	@docker build -t $(IMAGE_PREFIX)/$(NAME):$(VERSION) -t $(IMAGE_PREFIX)/$(NAME):latest -f Dockerfile -m 6g --build-arg TARGETS=release-all --target command .
	@docker push $(IMAGE_PREFIX)/$(NAME):latest
	@docker push $(IMAGE_PREFIX)/$(NAME):$(VERSION)
